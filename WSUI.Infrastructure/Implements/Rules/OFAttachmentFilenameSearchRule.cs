...
namespace OF.Infrastructure.Implements.Rules
{
    public class OFAttachmentFilenameSearchRule : OFBaseAttachmentSearchRule
    {
        ...
        protected override OFBody GetSearchBody()
        {
            ...
            {
                var query = new OFQueryBoolMust<OFQueryMatchPhrase<OFAttachmentSimpleFilenameMatchPhrase>>();
                body.query = query;
                foreach (var preparedCriteria in preparedCriterias)
                {
                    var term = new OFQueryMatchPhrase<OFAttachmentSimpleFilenameMatchPhrase>(new OFAttachmentSimpleFilenameMatchPhrase() {filename = preparedCriteria.Result});
                    query._bool.must.Add(term);
                }
                return body;
            }

            body.query = new OFQueryMatchPhrase<OFAttachmentSimpleFilenameMatchPhrase>(new OFAttachmentSimpleFilenameMatchPhrase() { filename = Query });
            return body;
        }
